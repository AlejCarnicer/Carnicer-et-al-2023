...
% % % 	analyzed_file = [analyzed_path_name analyzed_file_name{analyzed_file_index}];
    ...
    % select the values for the fitting
        ...
        for interval_number = 1:length(intervals)
            ...
            dataPointIndex2 = find((RESULTS(:,1) <= intervals(interval_number)+maxDist));
            dataPointIndex3 = find((RESULTS(:,1) >= intervals(interval_number)-maxDist));
            dataPointIndex4 = intersect(dataPointIndex3,dataPointIndex2);
             [minumumvalue dataPointIndex5] = min(abs(RESULTS(dataPointIndex4,1)-intervals(interval_number)));  
% %                     dataPointIndex2
% %                     dataPointIndex3
% %                     dataPointIndex4
...
% %                     RESULTS(dataPointIndex4(dataPointIndex5),3)
            dataPointIndex6 = find((RESULTS(:,1) <= intervals(interval_number)+0.1*maxDist));
            dataPointIndex7 = find((RESULTS(:,1) >= intervals(interval_number)-0.1*maxDist));
...
% %                 end;
                binned_data2 = [binned_data2; RESULTS(dataPointIndex8(dataPointIndex9),:) intervals(interval_number)];
                binned_data_index2 = [binned_data_index2; dataPointIndex8(dataPointIndex9)];
%                 elasticityData(q,j) = maxumumvalue;
            end;
            if ~isempty(dataPointIndex5)
                binned_data3 = [binned_data3; RESULTS(dataPointIndex4(dataPointIndex5),:) intervals(interval_number)];
                binned_data_index3 = [binned_data_index3; dataPointIndex4(dataPointIndex5)];
%             	elasticityData(q,j) = RESULTS(dataPointIndex4(dataPointIndex5),3); 
         	end;
        end
    fullcontactpointindex(1) = RESULTS(1,5)
    EModulus(2) = RESULTS(1,3)
    
    
    if isempty(binned_data_index)==1 %%|| isempty(binned_data_index2)==1 || isempty(binned_data_index2)==1
        contactpointindex = 400;
        EModulus(1) = 0;
    else
        contactpointindex(1) = RESULTS(binned_data_index,5);
        EModulus(1) = RESULTS(binned_data_index,3);
% %         fullcontactpointindex(2) = RESULTS(binned_data_index2,5);
% %         EModulus(3) = RESULTS(binned_data_index2,3);
% %         fullcontactpointindex(3) = RESULTS(binned_data_index3,5);
% %         EModulus(4) = RESULTS(binned_data_index3,3);
    end;
    
    h=figure('name', strcat(num2str(EModulus(1)),' and ',num2str(EModulus(2))))%,' and ',num2str(EModulus(3)),' and ',num2str(EModulus(4))))%,'visible','off');
...
%         plot(-rawdata{1,3}(contactpointindex(j))+rawdata{1,3}(contactpointindex(j)),rawdata{1,2}(1)-rawdata{1,2}(1),'rx');
        plot(-rawdata{1,3}(contactpointindex(j))+rawdata{1,3}(contactpointindex(j)),-rawdata{1,2}(1)+rawdata{1,3}(contactpointindex(j))*approachfitcoefficients(1,1)+approachfitcoefficients(1,2),'rx')
...
%     end;
    ...
    for j=1:length(intervals)
        ...
        [approachfit] = fitapproach (approachdata);
        approachfitcoefficients = coeffvalues(approachfit);
        plot(-rawdata{1,3}(1:fullcontactpointindex(k))+rawdata{1,3}(contactpointindex(j)),-rawdata{1,2}(1)+rawdata{1,3}(1:fullcontactpointindex(k))*approachfitcoefficients(1,1)+approachfitcoefficients(1,2),'Color','black')
        if k==1
            plot(rawdata{1,3}(contactpointindex(j))-rawdata{1,3}(fullcontactpointindex(k):end),-rawdata{1,2}(1)+4/3*EModulus(k+1)*sqrt(R)*(-rawdata{1,3}(fullcontactpointindex(k):end)+rawdata{1,3}(fullcontactpointindex(k))).^(3/2)+rawdata{1,3}(fullcontactpointindex(k):end)*approachfitcoefficients(1,1)+approachfitcoefficients(1,2)-4/3*EModulus(k+1)*sqrt(R)*(-rawdata{1,3}(fullcontactpointindex(k))+rawdata{1,3}(fullcontactpointindex(k))).^(3/2),'m')
            plot(-rawdata{1,3}(fullcontactpointindex(k))+rawdata{1,3}(contactpointindex(j)),-rawdata{1,2}(1)+rawdata{1,3}(fullcontactpointindex(k))*approachfitcoefficients(1,1)+approachfitcoefficients(1,2),'mx')
        elseif isempty(binned_data_index)==0
        if k==2
            plot(rawdata{1,3}(contactpointindex(j))-rawdata{1,3}(fullcontactpointindex(k):end),-rawdata{1,2}(1)+4/3*EModulus(k+1)*sqrt(R)*(-rawdata{1,3}(fullcontactpointindex(k):end)+rawdata{1,3}(fullcontactpointindex(k))).^(3/2)+rawdata{1,3}(fullcontactpointindex(k):end)*approachfitcoefficients(1,1)+approachfitcoefficients(1,2)-4/3*EModulus(k+1)*sqrt(R)*(-rawdata{1,3}(fullcontactpointindex(k))+rawdata{1,3}(fullcontactpointindex(k))).^(3/2),'g')
            plot(-rawdata{1,3}(fullcontactpointindex(k))+rawdata{1,3}(contactpointindex(j)),-rawdata{1,2}(1)+rawdata{1,3}(fullcontactpointindex(k))*approachfitcoefficients(1,1)+approachfitcoefficients(1,2),'gx')
        elseif k==3
            plot(rawdata{1,3}(contactpointindex(j))-rawdata{1,3}(fullcontactpointindex(k):end),-rawdata{1,2}(1)+4/3*EModulus(k+1)*sqrt(R)*(-rawdata{1,3}(fullcontactpointindex(k):end)+rawdata{1,3}(fullcontactpointindex(k))).^(3/2)+rawdata{1,3}(fullcontactpointindex(k):end)*approachfitcoefficients(1,1)+approachfitcoefficients(1,2)-4/3*EModulus(k+1)*sqrt(R)*(-rawdata{1,3}(fullcontactpointindex(k))+rawdata{1,3}(fullcontactpointindex(k))).^(3/2),'y')
            plot(-rawdata{1,3}(fullcontactpointindex(k))+rawdata{1,3}(contactpointindex(j)),-rawdata{1,2}(1)+rawdata{1,3}(fullcontactpointindex(k))*approachfitcoefficients(1,1)+approachfitcoefficients(1,2),'yx')
        end
        end
            %plot(-rawdata{1,3}(contactpointindex(j):end),rawdata{1,3}(contactpointindex(j):end)*approachfitcoefficients(1,1)+approachfitcoefficients(1,2),'Color','red')
        %rawdata{1,3}(contactpointindex(j):end);
