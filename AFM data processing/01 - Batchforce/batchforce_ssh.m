...
%Version 19/1/2016 Max Jakobs
%Version 22/7/2016 Alejandro Carnicer-Lombarte

%% Select the force curves to be analyzed
%2016-07-22 Edited by Alejandro Carnicer-Lombarte. Tweaked to remove
%uigetfile from script, in order to allow it to be run over ssh. It now
%instead automatically runs it on any and everything on the current
%directory (pwd). Be sure to only have force-curves here.

PathName = input('Select current directory (be sure to end in /, and use inverted commas) > ');           %Select current directory for use. PathName was obtained by uigetfile in original
dirData = dir(PathName);      % Get the data for the current directory
dirIndex = [dirData.isdir];  % Find the index for directories
FileNameT = {dirData(~dirIndex).name}';  % Get a list of the files
FileName = transpose(FileNameT);     %Transpose list of filenames to achieve same format as original
FilterIndex = 1;%set by original script, don't think it is used though
%PathName=strcat(pwd,'/'); %in the original format, path needs to end with '/'
